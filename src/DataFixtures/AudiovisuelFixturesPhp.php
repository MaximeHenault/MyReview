...
class AudiovisuelFixturesPhp extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        ...
        $breackingbad = new Audiovisuel();
        $breackingbad->setTitre('Breaking Bad');
        $breackingbad->setType('Série');
        $breackingbad->setRealisateur('Vince Gilligan');
        $breackingbad->setDateCreation(new \DateTime('2008-01-20'));
        $breackingbad->setAffiche('https://fr.web.img6.acsta.net/pictures/18/09/26/10/44/568567.jpg');
        $breackingbad->setDescription('Walter White, un professeur de chimie du lycée, apprend qu\'il est atteint d\'un cancer du poumon en phase terminale. Pour assurer l\'avenir financier de sa famille après sa mort, il décide de se lancer dans la fabrication et la vente de méthamphétamine avec l\'aide de son ancien élève, Jesse Pinkman. Cependant, leur entreprise illégale les entraîne dans un monde dangereux de crime et de violence.');
        
        $manager->persist($breackingbad);
        $manager->flush();
    }
}
